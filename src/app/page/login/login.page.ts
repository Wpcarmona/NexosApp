...
export class LoginPage implements OnInit {
  ...
  validationUserMessage ={
    email:[
      ...
      {type:"pattern", message:'El correo es incorrecto por favor intente de nuevo '}
    ],
    password:[
      {type:"required",message:'Por favor ingrese su contraseña'},
      {type:"minlength", message:'La contraseña debe contener 5 caracteres o más'}
        ...
        const userProfile = this.firestore.collection('profile').doc(resp.user.uid);
         userProfile.get().subscribe( result=>{
          if(result.exists){
            ...
            this.db.database.ref('datos/').child('user').child(this.authservice.getUID()).set({
              name: resp.user.displayName,
              email:resp.user.email,
              document:localStorage.getItem('document'),
              phone:localStorage.getItem('phone')
            })
            this.db.database.ref('datos/').child('user').child(this.authservice.getUID()).child('cuentaCorriente').child(localStorage.getItem('cuentac')).update({
              cuenta: localStorage.getItem('cuentac'),
              saldo: 0
            })
            this.db.database.ref('datos/').child('user').child(this.authservice.getUID()).child('cuentaAhorro').child(localStorage.getItem('cuentaa')).update({
